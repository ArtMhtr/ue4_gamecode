...
{
	LeftRailMeshComponent->SetRelativeLocation(FVector(0.0f, -LadderWidth * 0.5f, GetLadderHeight() * 0.5f));
	RightRailMeshComponent->SetRelativeLocation(FVector(0.0f, LadderWidth * 0.5f, GetLadderHeight() * 0.5f));
	...
	if (IsValid(StepsMesh))
	{
		...
		{
			StepsMeshComponent->SetRelativeScale3D(FVector(1.0f, LadderWidth / StepsWidth, 1.0f));
		}
	}

	StepsMeshComponent->ClearInstances();
	uint32 StepsCount = FMath::FloorToInt((GetLadderHeight() - BottomStepOffset) / StepsInterval);
	for (uint32 i = 0; i < StepsCount; ++i)
	{
		FTransform Transf(FVector(1.0f, 1.0f, BottomStepOffset + i * StepsInterval));
		StepsMeshComponent->AddInstance(Transf);
	}

	float BoxDepthExtent = GetLadderInteractionBox()->GetUnscaledBoxExtent().X;
	GetLadderInteractionBox()->SetBoxExtent(FVector(BoxDepthExtent, LadderWidth * 0.5f, GetLadderHeight() * 0.5f));
	GetLadderInteractionBox()->SetRelativeLocation(FVector(BoxDepthExtent, 0.0f, GetLadderHeight() * 0.5f));
